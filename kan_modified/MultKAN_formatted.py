...
class MultKAN(nn.Module):
    ...
    def update_grid_from_samples(self, x):
        ...
        for l in range(self.depth):
            self.get_act(x)
            msg = self.act_fun[l].update_grid_from_samples(self.acts[l])
            # tqdm.write(msg)   ### debug
        ...
        for l in range(neuron_depth - 1):
            ...
            for i in range(n):
                ...
                for j in range(n_next):
                    ...
                    # newax.imshow(im, alpha=alpha[l][j][i])
                    newax.add_patch(
                        plt.Rectangle((0, 0), 1, 1, fill=None, edgecolor="black")
                    )
                    newax.axis("off")
    ...
    def fit(
        ...
        if self.optimizer is None:
            if opt == "Adam":
                self.optimizer = torch.optim.Adam(self.get_params(), lr=lr)
            elif opt == "LBFGS":
    ...
    def module_weak(self, start_layer, chain):
        """
        specify network modules
        keeps id_in -> id_out_others connections
    ...
    def plot_activation(
        ...
        if metric == "backward":
            ...
            raise ValueError(
                f"Metric '{metric}' not recognized. Choose from 'backward', 'forward_n', 'forward_u'."
            )
        ...
        if l < 0 or l >= depth:
            raise ValueError(
                f"Layer index l={l} is out of range. Must be between 0 and {depth-1}."
            )
        ...
        if j < 0 or j >= self.width_out[l + 1]:
            raise ValueError(
                f"Output node index j={j} is out of range for layer {l+1}."
            )
        ...
        if symbolic_mask > 0.0 and numeric_mask > 0.0:
            color = "purple"
            alpha_mask = 1.0
        elif symbolic_mask > 0.0 and numeric_mask == 0.0:
            color = "red"
            alpha_mask = 1.0
        elif symbolic_mask == 0.0 and numeric_mask > 0.0:
            color = "black"
        ...
        ax.plot(
            input_sorted, output_sorted, color=color, lw=2, alpha=alpha * alpha_mask
        )
        ...
        if tick:
            ax.tick_params(
                axis="both", which="both", direction="in", labelsize=12 * scale
            )
