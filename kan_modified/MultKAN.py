...
class MultKAN(nn.Module):
    ...
    def plot_no_spline(self, folder=None, beta=3, metric='backward', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0):
        '''
        plot KAN without spline functions
        ...
        for l in range(neuron_depth - 1):
            ...
            for i in range(n):
                ...
                for j in range(n_next):
                    ...
                    newax = fig.add_axes([left, bottom, right - left, up - bottom])
                    # newax.imshow(im, alpha=alpha[l][j][i])
                    newax.add_patch(plt.Rectangle((0, 0), 1, 1, fill=None, edgecolor='black'))
                    newax.axis('off')
    ...
    def prune_node(self, threshold=1e-2, mode="auto", active_neurons_id=None, log_history=True):
        ...
        for i in range(len(self.acts_scale)):
            ...
            if i < len(self.acts_scale) - 1:
                ...
                model2.grid[i].data = model2.node_bias[i].data[active_neurons_up[i+1]]
    ...
    def plot_activation(self, l, i, j, folder=None, beta=3, metric='backward', scale=0.5, tick=False, sample=False, in_vars=None, out_vars=None, title=None, varscale=1.0):
        '''
        Plot a single activation function specified by layer l, input node i, and output node j.
        ...
        -----
            ...
            i : int
                The input node index within layer l (0-based).
            j : int
                The output node index within layer l+1 (0-based).
            folder : str, optional
                The folder to store the PNG file (default: "./figures").
            beta : float, optional
                Positive number controlling the transparency of the activation (default: 3).
            metric : str, optional
                The metric to use for transparency ('backward', 'forward_n', 'forward_u'; default: 'backward').
            scale : float, optional
                Controls the size of the diagram (default: 0.5).
            tick : bool, optional
                If True, display axis ticks (default: False).
            sample : bool, optional
                If True, overlay scatter points on the plot (default: False).
            in_vars : list of str or sympy.Expr, optional
                Names of input variables for labeling (default: None).
            out_vars : list of str or sympy.Expr, optional
                Names of output variables for labeling (default: None).
            title : str, optional
                Title of the plot (default: None).
            varscale : float, optional
                Scaling factor for variable labels (default: 1.0).
        ...
        >>> model = KAN(width=[2,3,1], grid=3, k=3, noise_scale=1.0)
        >>> x = torch.normal(0,1,size=(100,2))
        >>> model(x)  # Forward pass to obtain model.acts
        >>> model.plot_activation(l=0, i=1, j=2, title="Activation Function L0_I1_O2")
        '''
        if not self.save_act:
            print('Cannot plot since data are not saved. Set save_act=True first.')
            return

        # Forward pass to obtain activations if not already done
        if self.acts is None:
            if self.cache_data is None:
                raise Exception('Model hasn\'t seen any data yet.')
            self.forward(self.cache_data)

        # Attribute based on the specified metric
        if metric == 'backward':
            self.attribute()
        elif metric in ['forward_n', 'forward_u']:
            pass  # Assuming attribute handles these internally or they don't require extra steps
        else:
            raise ValueError(f"Metric '{metric}' not recognized. Choose from 'backward', 'forward_n', 'forward_u'.")
        ...
        depth = len(self.width) - 1
        if l < 0 or l >= depth:
            raise ValueError(f"Layer index l={l} is out of range. Must be between 0 and {depth-1}.")
        if i < 0 or i >= self.width_in[l]:
            raise ValueError(f"Input node index i={i} is out of range for layer {l}.")
        if j < 0 or j >= self.width_out[l+1]:
            raise ValueError(f"Output node index j={j} is out of range for layer {l+1}.")

        # Extract activation data
        input_acts = self.acts[l][:, i].cpu().detach().numpy()
        output_acts = self.spline_postacts[l][:, j, i].cpu().detach().numpy()

        # Sort activations based on input for better visualization
        rank = np.argsort(input_acts)
        input_sorted = input_acts[rank]
        output_sorted = output_acts[rank]

        # Determine mask and color
        symbolic_mask = self.symbolic_fun[l].mask[j][i]
        numeric_mask = self.act_fun[l].mask[i][j]
        ...
        # Calculate transparency based on the chosen metric
        def score2alpha(score):
            return np.tanh(beta * score)

        if metric == 'forward_n':
            scores = self.acts_scale[l][j][i]
        elif metric == 'forward_u':
            scores = self.edge_actscale[l][j][i]
        elif metric == 'backward':
            scores = self.edge_scores[l][j][i]
        else:
            raise ValueError(f"Metric '{metric}' not recognized.")

        alpha = score2alpha(scores.cpu().detach().numpy())

        # Create the plot
        fig, ax = plt.subplots(figsize=(8 * scale, 6 * scale))

        # Plot the activation function
        ax.plot(input_sorted, output_sorted, color=color, lw=2, alpha=alpha * alpha_mask)

        # Optionally add scatter points
        if sample:
            ax.scatter(input_sorted, output_sorted, color=color, s=100 * scale ** 2, alpha=alpha * alpha_mask)

        # Customize ticks
        if tick:
            ax.tick_params(axis="both", which="both", direction="in", labelsize=12 * scale)
            x_min, x_max = np.min(input_sorted), np.max(input_sorted)
            y_min, y_max = np.min(output_sorted), np.max(output_sorted)
        ...
        # Set plot aesthetics
        ax.patch.set_edgecolor('black' if alpha_mask == 1.0 else 'white')
        ax.patch.set_linewidth(1.5)
        ax.set_xlabel(f'Input Node {i} Activations', fontsize=12 * scale)
        ax.set_ylabel(f'Output Node {j} Activations', fontsize=12 * scale)

        # Add title if provided
        if title:
            ax.set_title(title, fontsize=14 * scale)

        # Save the figure
        if folder is not None:
            filename = f'sp_{l}_{i}_{j}.png'
            filepath = os.path.join(folder, filename)
            plt.savefig(filepath, bbox_inches="tight", dpi=300)
            plt.close(fig)  # Close the figure to free memory
            print(f"Activation function plot saved to {filepath}")
